...
class Table:
	def __init__(self, selector = False, selectorType = "Name"):	
		if selector:
			...
			if row == None:
				raise ValueError("Table '"+str(selector)+"' does not exist")
			self.id = row[0]
			self.name = row[1]
			self.dbname = row[2]
		else:
...
class Row:
	...
	def save(self, executeHandler = True):
		...
		if self.id:
			...
			db.commit()
			if executeHandler and self.table.name in tableEventHandlers and hasattr(tableEventHandlers[self.table.name], 'onUpdate'):
			...
			db.commit()
			self.id = c.lastrowid
			if executeHandler and self.table.name in tableEventHandlers and hasattr(tableEventHandlers[self.table.name], 'onInsert'):
	...
	def delete(self, executeHandler = True):
		if self.id:
			...
			db.commit()
			if executeHandler and self.table.name in tableEventHandlers and hasattr(tableEventHandlers[self.table.name], 'onDelete'):
